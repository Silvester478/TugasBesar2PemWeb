...
class Home extends BaseController
{
	...
	public function index()
	{
		...
		// query total product  
		$record_product = $db->query("SELECT COUNT(*) as total FROM products");
		$total_product = $record_product->getResult()[0];

		// query product < 5 pcs 
		$sisa = $db->query("SELECT
			...
			GROUP BY
				product_id
				having all_stock < 5");
		$need_buy = $sisa->getResult();


		$data = [
			'title_meta' 	=> view('partials/title-meta', ['title' => 'Dashboard']),
			'page_title' 	=> view('partials/page-title', ['title' => 'Dashboard', 'li_1' => 'NXT', 'li_2' => 'Dashboard']),
			'result'		=> $result,
